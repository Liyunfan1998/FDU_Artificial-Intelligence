...
class StayWestSearchAgent(SearchAgent):
    ...
    def __init__(self):
        ...
        self.searchType = lambda state: PositionSearchProblem(state, costFn,visualize=False)
...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # state described in form ((x,y),corners)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            corners = state[1]
            dx, dy = Actions.directionToVector(action)
            nextxy = (int(x + dx), int(y + dy))
            hitsWall = self.walls[nextxy[0]][nextxy[1]]  # bool

            if not hitsWall:
                if nextxy in corners:
                    corners = list(corners)
                    corners.remove(nextxy)
                    corners = tuple(corners)

                successors.append(((nextxy, corners), action, 1))
            ...
            for n in toVisit:
                if n not in visited:
                    ...
                    if remaining in distanceList:  # job of if, elif and else to check if distance was already computed
                        ...
                        dist = mazeDistance(item[1], n,
                                            gameState)  # find dist between dest and all other unvisited nodes, update in fringe
...
class FoodSearchProblem:
    ...
    # added by tomcat when playing with food heuristic
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def __init__(self, gameState):
        ...
        # self.food = gameState[1]

        # Store info for the PositionSearchProblem (no need to change this)
        self.walls = gameState.getWalls()
        # self.walls = walls
        self.startState = gameState.getPacmanPosition()
        # self.startState = gameState[0]
        self.costFn = lambda x: 1
        self._visited, self._visitedlist, self._expanded = {}, [], 0  # DO NOT CHANGE
